...
const UploadRecipe = () => { // Updated component name
    const [name, setName] = useState("");
    const [thumbnail, setThumbnail] = useState("");
    const [recipe, setRecipe] = useState(""); // Changed variable name for clarity
    const navigate = useNavigate();

    const submitRecipe = async () => { // Updated function name
        const data = { name, thumbnail, recipe }; // Use the recipe variable
        const response = await makeAuthenticatedPOSTRequest(
            "/create",
            data
        );
        if (response.err) {
            alert("Could not create recipe"); // Updated alert message
            return;
        }
        alert("Success");
        navigate("/home");
